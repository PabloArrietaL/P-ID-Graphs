...
export class ElementService {
  ...
  async createElement({
    ...
    return getManager()
      ...
      .then(async (data) => {
        ...
        data.detail = detail;
        const uptaded = this.updateElement(data.id, data, res);
        return res
          .status(200)
          .json({ message: 'Elemento creado correctamente' });
      })
      .catch((error) => {
        fs.unlink(file.path);
        return res
  ...
  async getAllElements() {
    const query = await getRepository(Element)
  ...
  async updateElement(id: number, element: IElement, res: Response) {
    try {
      await getManager().getRepository(Element).update({ id: id }, element);
      return res
        .status(200)
  ...
  async deleteElement(id: number, res: Response) {
    try {
      ...
      try {
        return getManager()
          ...
          .then(() => {
            try {
              fs.unlink(
                path.resolve(__dirname, `../../uploads/${element?.img}`)
              );
            } catch (_) {}
          ...
          .catch((__1) => {
            return res.status(500).json({
              message: 'El elemento se está utilizando en un proceso',
            });
          });
      } catch (error) {
        return res
      ...
      return res
        .status(500)
        .json({ message: 'Ha ocurrido un error', data: error_1.message });
    }
  }
}
