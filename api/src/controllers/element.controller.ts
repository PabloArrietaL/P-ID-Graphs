...
export class ElementController {
  ...
  @Post('/element')
  async post(
    @UploadedFile('img', { options: fileUploadOptions }) file: any,
    @Body() element: IElement,
    @Res() res: Response
  ) {
    const data = await this.dataService.elementService.createElement({
      element,
      file,
      res,
    });
  ...
  put(
    ...
    const data = this.dataService.elementService.updateElement(
      id,
      element,
      res
    );
