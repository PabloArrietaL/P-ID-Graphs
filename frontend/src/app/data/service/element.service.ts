...
import {
  ...
  ElementEdit,
  ElementDetail,
} from "@data/schema/element.interface";
...
export class ElementService {
  ...
  proccesElement(controlled: any) {
    ...
    const arrW2 = [];
    const arrW3 = [];
    const status1 = controlled.first_status.name;
    const status2 = controlled.second_status.name;
    const status1FL = status1.slice(0, 1);
    const status2FL = status2.slice(0, 1);
    if (controlled.detail.third_status === null) {
      if (controlled.detail.first_status.second_status.checked === true) {
        select1to2 = status1FL.toUpperCase() + "to" + status2FL.toUpperCase();
        arrW2.push(select1to2);
      }
      if (controlled.detail.second_status.first_status.checked === true) {
    ...
    if (controlled.detail.third_status !== null) {
      const status3 = controlled.third_status.name;
      ...
      if (controlled.detail.third_status.first_status.checked === true) {
        select3to1 = status3FL.toUpperCase() + "to" + status1FL.toUpperCase();
        arrW3.push(select3to1);
      }
      if (controlled.detail.third_status.second_status.checked === true) {
        select3to2 = status3FL.toUpperCase() + "to" + status2FL.toUpperCase();
        arrW3.push(select3to2);
      }
      if (controlled.detail.first_status.third_status.checked === true) {
        select1to3 = status1FL.toUpperCase() + "to" + status3FL.toUpperCase();
        arrW3.push(select1to3);
      }
      if (controlled.detail.second_status.third_status.checked === true) {
